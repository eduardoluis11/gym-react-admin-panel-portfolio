import React, { useCallback, useEffect, useRef, useState } from 'react';  // Este modulo NO está causando el bug del "location not found".
import Head from 'next/head'; // Este modulo NO está causando el bug del "location not found".
import NextLink from "next/link"; // Este modulo NO está causando el bug del "location not found".
...
import {
  ...
  Typography,
  CardContent
} from '@mui/material'; // Este modulo NO está causando el bug del "location not found".
import { DateTimePicker } from '@mui/x-date-pickers'; // Este modulo NO está causando el bug del "location not found".
import { clientApi } from '../../../../api/client-api'; // Este modulo NO está causando el bug del "location not found".

// Esto me agrega la Disposición con el Navbar (tanto el de arriba como el de la izquierda)
import { DashboardLayout } from '../../../../components/dashboard/dashboard-layout';  // Este modulo NO está causando el bug del "location not found".

// ESTE ES EL MODULO QUE ME ESTA CAUSANDO EL BUG DEL "location not found". REVISAR el archivo
// "client-list-for-uploading-documents.js".
import { ClientListTable } from '../../../../components/client/client-list-for-uploading-documents';
// FIN DEL MODULO QUE ME ESTA CAUSANDO EL BUG DEL "location not found".
...
// Uno de los módulos anteriores me está causando el bug del "location not found". LOS DE ABAJO NO DEBERIAN.
...
// /* Esto es para intentar ver si window o location esta definido o no.
//
//  */
// if (typeof window !== 'undefined') {
//   console.log('ESTO ESTA FUERA DEL INDEX = (). window is defined');
//   if (typeof location !== 'undefined') {
//     console.log('ESTO ESTA FUERA DEL INDEX = (). location is defined:', location.href);
//   } else {
//     console.log('location is not defined. ESTO ESTA FUERA DEL INDEX = ()');
//   }
// } else {
//   console.log('window is not defined. ESTO ESTA FUERA DEL INDEX = ()');
// } // Fin del intento de ver si window o location esta definido o no.
...
* en Django.
*
* BUG: AQUI TENGO UN BUG EN EL QUE, si entro a esta página y refresco la página, me sale un bug que dice "location not
* found", el cual, además, NO me deja desplegar esta web app en Vercel. TENGO QUE URGENTEMENTE CORREGIR ESTE BUG.
*
* La función useCallBack() podría ser uno de los módulos que me puede estar causando el bug del "location not found",
* ya que este modulo no se encuentra ni en upload-documents/ ni en send-email/.
* */
...
const Index = () => {
  ...
  // Esto es paara intentar ver si window o location esta definido o no.
  // ESTO NUNCA SE EJECUTA CUANDO ME SALE EL BUG DEL "location not found". Es decir, el bug ocurre ANTES del Index = ().
  useEffect(() => {
    try {
      if (typeof window !== 'undefined') {
        console.log('window is defined');
        if (typeof location !== 'undefined') {
          console.log('location is defined:', location.href);
        } else {
  ...
  }, []); // Fin del useEffect que intenta ver si window o location esta definido o no.
  ...
  /* Esto creo que usa el Google Tag Manager para Google Analytics.

  Esto NO está causando el bug del "location not found".
  */
  useEffect(() => {
  ...
  /* ESTO ME PUEDE SERVIR para agarrar el ID de los clientes!!
  *
  * Nota que aqui se llama el isMounted, por lo que creo que esta función es una de las que necesita la funcion
  * useMounted().
  * */
  const getClients = useCallback(async () => {
    try {
      ...
      // // Tal vez DESACTIVE esto despues
      // setClients(data);
...
// }, []); // Tal vez DESACTIVE esto DESPUES
  ...
  /* Esto al parecer llama las funciones para coger todos los clientes, productos, suscripciones y bonos.
  *
  * Esto NO está causando el bug del "location not found".
  * */
  useEffect(() => {
    ...
    // // DEBUGGEO: esto me debe imprimir algo sobre el "location" en el inspector del navegador.
    // if (typeof window !== "undefined") {
    //   // This code will only run in the browser
    //   console.log("Esto es lo que imprime el location.href: ");
    //   console.log(location.href); // Example of accessing `location`
    // }
  },
  []);
  ...
  * */
  ...
  /* Estas 4 variables "const" me están dando una advertencia que dice "type string cannot be undefined".

  Veré si, corrigiéndolas, se arregla el bug del "location not found".
  * */
  ...
  };  // Fin de las 4 variables "const" que me están dando una advertencia que dice "type string cannot be undefined".
  ...
  *
  * NECESITO USAR el snippet de "router.isReady" con el useEffect para que se termine de enviar el documento
  * seleccionado a los clientes seleccionados.
  * */

  // REACTIVAR DESPUES
  // Esto guarda en una variable la función useRouter() de Next.js
  ...
  // REACTIVAR DESPUES si lo que quiero hacer no funciona
  // Esto me permite terminar de enviar el documento seleccionado a los clientes seleccionados.
  useEffect(() => {

...
// // ESTA LINEA DE CODIGO ME ESTA CAUSANDO EL ERROR DE "location is not defined" en
// // /documents/send-to-client/id_del_documento . DEBO CORREGIR ESTE SNIPPET. SI ELIMINO ESTE SNIPPET, NO ME SALE EL
// // ERROR, por lo que puedo refrescar la pagina sin problemas.
...
*
* BUG: ESTE ES EL ARCHIVO QUE ME ESTÁ CAUSANDO EL ERROR DE "location is not defined" en
* /documents/send-to-client/id_del_documento ! Creo que el snippet que me está dando problemas es el
* "router.push('/documents')". Voy a ver si le pongo un "if window is not undefined" para evitar el error.
 */
...
export const ClientListTable = (props) => {
  ...
  // Esto me dejará usar el useRouter() para redirigir al usuario a la página de /documents usando router.push().
  ...
  const enviarDocumentoAClientesSeleccionados = (documentId, event) => {
    ...
    axios.post(`${process.env.NEXT_PUBLIC_API_ROOT}/api/send-document-to-members/`, dataToSend)
      .then(response => {
        ...
        // Esto llama a la funcion de useRouter(), y me deja redirige a la página de /documents sin causar bugs.
        router.push('/documents');

        // // Esto debe redirigir al usuario a /documents sin darme el error de "location is not defined".
        // ESTO ME GENERA UN BUG en el que sale "documento no enviado" y "documento enviado" al mismo tiempo, e igual no
        // redirige al usuario a la página de /documents.
        // useRouter().push('/documents');

        // // ESTO DEBE CORREGIR EL BUG DE "location is not defined" en /documents/send-to-client/id_del_documento.
        // // NO: ESTO NO CORRIGIÓ EL BUG DE "location is not defined".
